...
//        $vnp_HashSecret = "KDASWVSUZEFSCKKWZPRQEEKFIAHQZULP"; //Chuỗi bí mật
    ...
    public function createForStoragePackage(Request $request)
    {
        $user = $request->user;
        $storage = StoragePackage::find($request->storagePackage_id);

        Session::put('storagePackage_id', $request->storagePackage_id);
        Session::put('user_id', $user->user_id);
        Session::put('noMonth', $request->selectedMonth);
        session(['cost_id' => $request->id]);
        session(['url_prev' => url()->previous()]);
//        $vnp_TmnCode = "8ZOGO9RW"; //Mã website tại VNPAY
//        $vnp_HashSecret = "KDASWVSUZEFSCKKWZPRQEEKFIAHQZULP"; //Chuỗi bí mật
        ...
        $vnp_Url = "http://sandbox.vnpayment.vn/paymentv2/vpcpay.html";
        $vnp_Returnurl = "http://goodlearning.com/storagePackage/callbackVPN";
        $vnp_TxnRef = date("YmdHis"); //Mã đơn hàng. Trong thực tế Merchant cần insert đơn hàng vào DB và gửi mã này sang VNPAY
        $vnp_OrderInfo = "Thanh toán hóa đơn phí dich vụ";
        $vnp_OrderType = 'billpayment';
        $vnp_Amount = ($storage->price*$request->selectedMonth) * 100;
        $vnp_Locale = 'vn';
        $vnp_IpAddr = request()->ip();
    ...
    public function callbackForStoragePackage(Request $request) {
        $user_id = Session::get('user_id');
        $noMonth = Session::get('noMonth');
        $storagePackage_id = Session::get('storagePackage_id');
        Session::put('user_id', 0);
        Session::put('noMonth', 0);
        if($request->vnp_ResponseCode == '00') {

            $userStorage = StorageUser::find($user_id);
            if($userStorage) {
                $json = json_decode($userStorage->json_storagePackage);
                $userStorage->json_storagePackage = json_encode(array_push($json,StoragePackage::find($storagePackage_id)));
                $userStorage->noMonth += $noMonth;
                $userStorage->save();
            } else {
                $userStorage = new StorageUser();
                $userStorage->user_id = $user_id;
                $userStorage->json_storagePackage = json_encode(StoragePackage::find($storagePackage_id));
                $userStorage->noMonth = $noMonth;
                $userStorage->save();
            }
            ...
            $bill->vnp_TransactionNo =$request->vnp_TransactionNo;
            $temp = [
                'storagePackage_id' => $storagePackage_id,
                'name' => StoragePackage::find($storagePackage_id)->name,
                'priceTier' => $request->vnp_Amount / 100,
                'course_updated' => $userStorage->updated_at
            ];
            $bill->currentInfo = json_encode($temp);
            $bill->save();
            return redirect('http://localhost:8081/mypage?state='.$request->vnp_ResponseCode);
        }
    }
