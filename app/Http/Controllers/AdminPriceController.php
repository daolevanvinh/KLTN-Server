...
class AdminPriceController extends BaseController{
    ...
    public function insertMoneytype(Request $request){
        $moneytypeNewValue = $request->moneytypeNewValue;
        $moneytype = MoneyType::where("moneyType_id","=",$moneytypeNewValue)->get();

        if(count($moneytype) == 0){
            $newmoneytype = new MoneyType();
            $newmoneytype->moneyType = $moneytypeNewValue;
            $newmoneytype->disable = false;
            $newmoneytype->save();
            return [
                "RequestSuccess" => true,
                "msg" => "New pricetier was added successfully !",
